...
def main(name, ip):
        ...
        while True:
            try:
                ...
                if "," in message:
                    ...
                    start_pos = levels[0]["start_positions"][role]
                    my_player = Player(player_id, name, role, colors[role], *start_pos)
                    encryption_key, _ = client_socket.recvfrom(BUFFER_SIZE)
                else:
    ...
    while running:
        ...
        client_socket.sendto(xor(data, encryption_key), (SERVER_IP, SERVER_PORT))
